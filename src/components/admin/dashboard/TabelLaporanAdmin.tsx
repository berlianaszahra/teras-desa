        ...
        <div className="absolute top-[37px] left-0 w-full border border-black rounded-b-lg overflow-hidden z-10">
          {statusOptions.map((opt) => (
            <button
              ...
              className="w-full px-4 py-[9px] text-base font-medium text-[#190B02] text-left border-b border-black last:border-b-0 hover:opacity-80"
...
export default function TabelLaporanAdmin() {
  ...
  async function updateStatus(id: string, status: ReportStatus) {
    try {
      ...
      // Update state lokal secara optimistis untuk menghindari refetch penuh.
      setData((prev) =>
  ...
  const columns = ["No", "Judul", "Proyek", "Tanggal Laporan", "Lokasi", "Status", "Aksi"];

  return (
    <div className="w-full bg-[#E6E5D9] rounded-[30px] p-4 md:p-6 flex flex-col gap-4">
      <h2 className="text-xl font-semibold text-[#190B02]">
          ...
          {loading ? (
            ...
            data.map((row, index) => (
              <div key={row.id} className="flex flex-row border-b border-[#190B02] last:border-b-0 bg-[#FDF5E3]">
                ...
                <div className="w-[60px] flex items-center justify-center px-4 py-4 text-black">
                  {(page - 1) * limit + index + 1}
                </div>

                <div className="flex-1 flex items-center px-4 py-4 text-black">
                  {row.title}
                </div>

                <div className="w-[160px] flex items-center px-4 py-4 text-black">
                  {row.project?.title || "-"}
                </div>

                <div className="w-[166px] flex items-center px-4 py-4 text-black">
                  {new Date(row.createdAt).toLocaleDateString('id-ID', {
                    day: 'numeric', month: 'long', year: 'numeric'
                  })}
                </div>

                <div className="w-[157px] flex items-center px-4 py-4 text-black">
                  {row.location}
                </div>
      ...
      {!loading && total > 0 && (
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mt-2">
            ...
            {Array.from({ length: totalPages }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-[40px] h-[31px] flex items-center justify-center border border-[#3F5210] rounded-[10px] ${
                    page === p ? "bg-[#3F5210] text-white" : "bg-[#ECEEE7] text-black"
                  }`}
                >
                  {p}
                </button>
